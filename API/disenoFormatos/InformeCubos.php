<?php 
	...
	class InformeCubos extends MyPDF{
		...
		*/
		...
		function generateCellsCampos(){
			...
			$lado2 = 'LADO 2 EN';
			$tam_lado2Ancho = $this->GetStringWidth($lado2) + 3;
			$tam_lado2Alto = 0.75*($tam_font_CellsSmall - 2.5);
			$lado2 = $lado2."\n".'cm';

	
			$lado1 = 'LADO 1 EN';
			$tam_lado1Ancho = $this->GetStringWidth($lado1) + 3;
			$tam_lado1Alto = 0.75*($tam_font_CellsSmall - 2.5);
			$lado1 = $lado1."\n".'cm';
			...
			$tam_elementoAlto = 1.5*(2*($tam_font_Cells) - 6);

			$tam_separador = 259.3975;
			...
			$this->cellsTables = array(
											...
											'lado2' 						=> $lado2,
											'tam_lado2Ancho' 				=> $tam_lado2Ancho,
											'tam_lado2Alto' 				=> $tam_lado2Alto,

											'lado1' 						=> $lado1,
											'tam_lado1Ancho' 				=> $tam_lado1Ancho,
											'tam_lado1Alto' 				=> $tam_lado1Alto,
		...
		*/
		function putInfo($infoFormato){
			...
			*/
			...
			$this->Ln($this->cellsInfo['tam_font_left'] - 1);
		}
		...
		function putTables($infoFormato,$regisFormato,$infoU,$id_registrosCampo){
			...
			$this->SetX(-($this->cellsTables['tam_resistenciaAncho'] + 10));

			//Procentaje Resistencia
			$posicion_x = $this->GetX();
			...
			//Abajo de Escpecimenes
			//Area
			$this->SetY($posicion_y + (1.5*($this->cellsTables['tam_font_CellsSmall'] - 2.5)));	
			...
			//Lado2
			$this->SetY($posicion_y + (1.5*($this->cellsTables['tam_font_CellsSmall'] - 2.5)));	
			$this->SetX($posicion_x - $this->cellsTables['tam_lado2Ancho']); 
			$posicion_x = $this->GetX();
			$this->multicell($this->cellsTables['tam_lado2Ancho'],$this->cellsTables['tam_lado2Alto'],$this->cellsTables['lado2'],1,'C');

			//Lado1
			$this->SetY($posicion_y + (1.5*($this->cellsTables['tam_font_CellsSmall'] - 2.5)));	
			$this->SetX($posicion_x - $this->cellsTables['tam_lado1Ancho']); 
			$posicion_x = $this->GetX();
			$this->multicell($this->cellsTables['tam_lado1Ancho'],$this->cellsTables['tam_lado1Alto'],$this->cellsTables['lado1'],1,'C');
			...
			$tam_separador = array_pop($this->arrayCampos);
			...
			if(count($arrayGrupo1)!=0){
				$this->error = $this->putInfoTablesWithPositionInformes($tam_elementoAncho + 10,$arrayGrupo1,$grupos,$this->cellsTables['tam_font_CellsRows'],$this->arrayCampos,$this->cellsTables['tam_cellsTablesAlto'],$id_registrosCampo,'cubos');
			...
			if(count($arrayGrupo2)!=0){
				$this->error = $this->putInfoTablesWithPositionInformes($tam_elementoAncho + 10,$arrayGrupo2,$grupos,$this->cellsTables['tam_font_CellsRows'],$this->arrayCampos,$this->cellsTables['tam_cellsTablesAlto'],$id_registrosCampo,'cubos');
				$endDown_table = $this->GetY();
				//Imprimimos el "elemento muestreado"
				$this->SetXY($posicion_x,$posicion_y);
		...
		function putCaracCampos(){
			...
			//Lineas de codigo para probar las entradas de las celda de localizacion
			$string = 'Prueba de localización, el motivo de esta prueba es ver si no corta las palabras en cada salto de linea, de ser asi contacte al administrador o al responsable de la creación de los formatos, ponga un reporte o contacte al 01 800 TEC4U.¡Que tenga un excelente día!. :) ';

			$this->multicell($tam_elementoAncho,$this->cellsTables['tam_cellsTablesAlto'],'Cadena:'.utf8_decode($string).'-Cantidad de caractares:'.strlen($string),'L,T','C');
			...
			$this->cell(0,$this->cellsTables['tam_cellsTablesAlto']-2,'',1,2,'C');
