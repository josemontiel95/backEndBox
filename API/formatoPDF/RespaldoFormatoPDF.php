<?php 
	...
	*/
	...
	$pdf->SetFont($fuente_titulo,'B',8);
	$pdf->SetX(-$tam_tituloInforme-10);
	$pdf->Cell($tam_tituloInforme,5,utf8_decode($titulo_informe),1,'C');
	...
	$pdf->Cell($pdf->GetStringWidth($nomCli)+2,5,utf8_decode($nomCli),0);
	...
	$pdf->SetX(-($tam_falla + 10));
	$posicion_x = $pdf->GetX();
	$pdf->multicell($tam_falla,4.5,utf8_decode($falla.'N°'),1,'C');
	//Resistencia
	...
	$pdf->SetY($posicion_y);
	$pdf->SetX($posicion_x - $tam_resistencia);
	$posicion_x = $pdf->GetX();
	$pdf->multicell($tam_resistencia,4.5,utf8_decode('% DE'."\n".$resistencia),1,'C');


	//F'c de proyecto

	$proyecto = 'PROYECTO';
	$tam_proyecto = $pdf->GetStringWidth($proyecto)+3;
	$pdf->SetY($posicion_y);
	$pdf->SetX($posicion_x - $tam_proyecto);
	$posicion_x = $pdf->GetX();
	$pdf->multicell($tam_proyecto,3,utf8_decode("F'c"."\n".'PROYECTO'."\n".'(kg/cm²)'),1,'C');
